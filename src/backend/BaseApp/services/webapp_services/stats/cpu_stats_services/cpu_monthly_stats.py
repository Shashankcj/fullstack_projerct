...
def cpu_monthly_stats(request, agent_uuid):
    try:
        cache_key = f"cpu_monthly_stats_{agent_uuid}"
        ...
        data = get_cpu_monthly_stats(cpu_qs.first())
        response_data = build_unified_response('cpu', cpu_qs, "monthly", data)
        ...
        logger.error(f"Error in cpu_monthly_stats for agent {agent_uuid}: {str(e)}")
        return Response({
            "status": "error",
