...
def cpu_weekly_stats(request, agent_uuid):
    try:
        cache_key = f"cpu_weekly_stats_{agent_uuid}"
        ...
        data = get_cpu_weekly_stats(cpu_qs.first())
        response_data = build_unified_response('cpu', cpu_qs, "weekly", data)
        ...
        logger.error(f"Error in cpu_weekly_stats for agent {agent_uuid}: {str(e)}")
        return Response({
            "status": "error",
