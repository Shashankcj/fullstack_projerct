...
def disk_weekly_stats(request, agent_uuid):
    try:
        cache_key = f"disk_weekly_stats_{agent_uuid}"
        ...
        for disk in disk_qs:
            try:
                disk_data = get_disk_weekly_stats(disk)
                data[str(disk.uuid)] = disk_data
            except Exception as disk_error:
                logger.error(f"Error getting weekly stats for disk {disk.uuid}: {str(disk_error)}")
                data[str(disk.uuid)] = {}
        
        response_data = build_unified_response('disk', disk_qs, "weekly", data, is_multi_resource=True)
        ...
        logger.error(f"Error in disk_weekly_stats for agent {agent_uuid}: {str(e)}")
        return Response({
            "status": "error",
