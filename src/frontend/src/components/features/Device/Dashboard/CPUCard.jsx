...
export const CPUCard = ({ isDarkMode }) => {
  const { agentId } = useParams();
  const navigate = useNavigate();
  const { refreshInterval } = useRefreshSettings();

  const { data: cpuData, loading } = useMetricData({
    endpoint: `/device/cpu-utilization/${agentId}/`,
    refreshInterval,
    params: { interval: refreshInterval > 0 ? refreshInterval : 1 },
  });

  const isEmpty = !loading && (!cpuData || cpuData.length === 0);
  const handleCardClick = () => navigate(`/devices/${agentId}/mon/cpu`);
  ...
  return (
    <MetricCard
      ...
      Icon={Cpu}
      isEmpty={isEmpty}
      emptyText="No CPU data available"
    ...
    >
      ...
      <div className="flex-1 min-h-0" onClick={(e) => e.stopPropagation()}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={cpuData} margin={{ left: 11, right: 11 }}>
